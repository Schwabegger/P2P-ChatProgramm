...
namespace Basics.UserControls
{
    ...
    public partial class IpField : INotifyPropertyChanged
    {
        ...
        private void FirstByteTextBox_OnPreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (!Paste(sender, e))
                ShiftTextBoxFocus((TextBox)sender, e);
        }
        private void SecondByteTextBox_OnPreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (!Paste(sender, e))
                ShiftTextBoxFocus((TextBox)sender, e);
        }
        private void ThirdByteTextBox_OnPreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (!Paste(sender, e))
                ShiftTextBoxFocus((TextBox)sender, e);
        }
        private void FourthByteTextBox_OnPreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (!Paste(sender, e))
                ShiftTextBoxFocus((TextBox)sender, e);
        }
        ...
        private static void ShiftTextBoxFocus(TextBox textBox, KeyEventArgs e)
        {
            switch (e.Key)
            {
                ...
                case Key.Left:
                    {
                        if (textBox.CaretIndex == 0 && textBox.Name != nameof(FirstByteTextBox))
                        {
                            textBox.MoveFocus(new TraversalRequest(FocusNavigationDirection.Left));
                            TextBox focusedControl = (TextBox)Keyboard.FocusedElement;
                            focusedControl.SelectionStart = focusedControl.Text.Length;
                        }
                        break;
                    }
                default:
                    if (textBox.Text.Length == 3 || textBox.Text.ToByte() >= 26)
                        textBox.MoveFocus(new TraversalRequest(FocusNavigationDirection.Right));
                    break;
            }
        }
        ...
        private bool Paste(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.V && (Keyboard.Modifiers & ModifierKeys.Control) == ModifierKeys.Control)
            {
                ...
                if (Clipboard.ContainsText(TextDataFormat.Text))
                {
                    if (Clipboard.ContainsText(TextDataFormat.Text))
                    {
                        ...
                        IpAddress = clipboardText;
                        FourthByteTextBox.Focus();
                        FourthByteTextBox.SelectionStart = FourthByteTextBox.Text.Length;
                        return true;
                    }
                }
            }
